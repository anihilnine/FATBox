    ...
    public class PreviewBuilder
    {
        ...
        void LoadWaterShader()
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                Texture2D tmp = tl.LoadTexture(scmapData.Water.WaveTextures[i].TexPath);
            ...
            ShaderResourceView skyCube = tl.LoadTextureCube(scmapData.TexPathSkyCubemap);
            WaterFX.GetVariableByName("SkyMap").AsResource().SetResource(skyCube);

            //Water Fresnel Sampler
            Texture2D fresnel = tl.LoadTexture(Resources.FresnelTexture_dds);
            ...
            //WaterMap
            Texture2D wm = tl.LoadTexture(scmapData.WatermapTex);
        ...
        void LoadTerrainAndFrameShaders()
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                {
                    Texture2D tmp = tl.LoadTexture(scmapData.Layers[i].PathTexture);
                    ShaderResourceView srv = new ShaderResourceView(device, tmp);
            ...
            for (int i = 0; i < 9; i++)
            {
                ...
                {
                    Texture2D tmp2 = tl.LoadTexture(scmapData.Layers[i].PathNormalmap);
            ...
            //Texture Data
            Texture2D tm1 = tl.LoadTexture(scmapData.TexturemapTex);
            ...
            Texture2D tm2 = tl.LoadTexture(scmapData.TexturemapTex2);
            ...
            Texture2D ss = tl.LoadTexture(Resources.ShadowTexture_dds);
            ...
            //Bicubic Sampler
            Texture2D bicubic = tl.LoadTexture(Resources.BicubicRamp_dds);
